...
export const DDataTable: React.FC<DDataTableInterface> = ({ cell, items, onPress }) => {
                    ...
                    <DataTable>
                        <DataTable.Header>
                            {cell.map((header, index) => (
                                <DataTable.Title style={[{ width: 100 }]} key={index}>{header.displayName}</DataTable.Title>
                            ))}
                        </DataTable.Header>

                        {items.slice(from, to).map((item, rowIndex) => (
                            <TouchableOpacity key={rowIndex} onPress={() => onPress && onPress(item)}>
                                <View>
                                    <DataTable.Row>
                                        {cell.map((header, cellIndex) => (
                                            <DataTable.Cell style={[{ width: 100 }]} key={cellIndex}>{item[header.key]}</DataTable.Cell>                                        ))}
                                    </DataTable.Row>
                                </View>
                            </TouchableOpacity>
                        ))}
